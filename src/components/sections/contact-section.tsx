          ...
          <div className="relative grid gap-0 lg:grid-cols-[1.08fr_1fr]">
            <CardHeader className="space-y-6 border-b border-slate-800/80 p-7 sm:p-10 lg:border-b-0 lg:border-r lg:border-slate-800/80">
              ...
              <div className="space-y-3">
                <CardTitle className="text-2xl font-semibold leading-tight text-white sm:text-3xl lg:text-4xl">
                  Ready for a new project?
                </CardTitle>
                <CardDescription className="max-w-xl text-base leading-relaxed text-slate-300 break-words">
                ...
                {channels.map((channel, index) => (
                  <FadeContent key={channel.label} duration={550} delay={index * 110} translateY={16}>
                    <a
                      ...
                      className="group flex flex-col gap-3 rounded-2xl border border-slate-700/80 bg-slate-900/55 px-4 py-4 transition-all duration-300 hover:-translate-y-0.5 hover:border-slate-500/80 hover:bg-slate-800/65 sm:flex-row sm:items-center sm:justify-between"
                    >
                      <span className="inline-flex min-w-0 items-center gap-3">
                        ...
                        <span className="min-w-0">
                          ...
                          <span className="block text-xs text-slate-400 break-words sm:truncate">{channel.detail}</span>
                        </span>
                      </span>
                      ...
                      <span className="inline-flex w-full items-center justify-between gap-2 text-left sm:w-auto sm:shrink-0 sm:justify-end sm:text-right">
                        <span className="min-w-0 max-w-full break-all text-sm text-slate-300 sm:max-w-[12rem] sm:truncate sm:break-normal">{channel.value}</span>
